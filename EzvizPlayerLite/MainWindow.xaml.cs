...
namespace EzvizPlayer
{
    public partial class MainWindow : Window
    {
        ...
        private Border CreateVideoPanel(DeviceConfig dev, Config config)
        {
            ...
            string statusText = isRtsp ? "不支持" : (dev.Mode == "rec" ? "回放" : "直播中");
            ...
            var statusDot = new Ellipse
            {
                Width = 5,
                Height = 5,
                Fill = isRtsp ? new SolidColorBrush(Colors.Gray) :
            ...
            try
            {
                if (isRtsp)
                {
                    contentGrid.Children.Add(CreateUnsupportedView("RTSP 播放功能已在 Lite 版本中移除。"));
                }
                else
                {
        ...
        private Grid CreateUnsupportedView(string message)
        {
            var grid = new Grid { Background = new SolidColorBrush(Color.FromRgb(5, 16, 32)) };
